...
function trustLevelItem(trustLevel) {
  const name = trustLevel
    ? trustLevel.name()
    : app.translator.trans(
        "askvortsov-trust-levels.admin.trust_level_page.create_trust_level_button"
      );
  const iconName = trustLevel ? trustLevel.group().icon() : "fas fa-plus";
  const style = trustLevel
    ? { "background-color": trustLevel.group().color(), color: "white" }
    : "";
  return (
    <div
...
export default class TrustLevelsPage extends ExtensionPage {
  oninit(vnode) {
    ...
    app.store.find("trust_levels").then(() => {
      this.loading = false;
      m.redraw();
    });
  }
      ...
      <div className="TrustLevels">
        <div className="container">
          <div className="ExtensionsWidget-list TrustLevels-list">
            <p className="TrustLevels-list-heading">
              {app.translator.trans(
                "askvortsov-trust-levels.admin.trust_level_page.list_heading"
              )}
            </p>
            <div className="ExtensionList">
              {[
                ...app.store.all("trust_levels").map(trustLevelItem),
                trustLevelItem(),
              ]}
            </div>
          </div>
          <div className="TrustLevels-footer">
            <p>
              {app.translator.trans(
                "askvortsov-trust-levels.admin.trust_level_page.instructions_header"
              )}
            </p>
            <ul>
              {app.translator.trans(
                "askvortsov-trust-levels.admin.trust_level_page.instructions_text"
              )}
            </ul>
          </div>
        </div>
